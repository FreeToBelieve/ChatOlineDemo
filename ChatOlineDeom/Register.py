...
def register_check(register_ui, entry_num, entry_name, entry_password, entry_cofirm_password): #注册校验：包括2次密码输入校验、用户名格式校验
...
def return_top(register_ui): #返回登陆界面
    register_ui.destroy()
    Login.Login()


def check_num(entry_num): #以正则表达式的方式校验用户名格式，如果错误返回False
    num = entry_num.get()
    if 6 <= len(num) <= 9:
